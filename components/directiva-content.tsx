...
import { UserCircle, MapPin, ExternalLink, Info } from "lucide-react"
  ...
  return (
    <div className="bg-card rounded-2xl overflow-hidden shadow-sm border hover:shadow-md transition-shadow">
      ...
      <div className="relative h-48 w-full bg-muted">
        ...
        {/* Role Badge - High contrast for visibility over images */}
        {member.role && (
          <div className="absolute top-3 left-3">
            <Badge className="bg-foreground/90 text-background font-medium shadow-md">
              {member.role}
            </Badge>
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <p className="text-sm text-foreground/70 mb-1">{member.churchName}</p>
        <p className="text-xs text-foreground/60 mb-4">{formatPhoneForDisplay(member.phone)}</p>
  ...
  return (
    <div className="px-4 py-6">
      <div className="max-w-4xl mx-auto">
        ...
        {/* Info Banner */}
        <div className="bg-muted/50 rounded-2xl p-4 mb-6 border flex items-start gap-3">
        ...
        {/* Directiva Grid - Centered when few cards */}
