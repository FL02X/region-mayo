...
function getRecorridoDateParts(date: Date) {
...
export function RecorridoContent({
  ...
  const startDateParts = startDate ? getRecorridoDateParts(startDate) : null;
  const endDateParts = endDate ? getRecorridoDateParts(endDate) : null;
            ...
            <div className="mt-10 w-fit border border-border bg-paper-dark px-2 pr-4 py-4 ">
              <div className="flex items-center gap-4">
                ...
                <div className="flex min-w-0 flex-1 flex-col items-start gap-2 uppercase">
                  <button type="button" onClick={handleOpenRegistration} disabled={!canOpenRegistration} className="inline-flex min-h-10 items-center justify-center gap-1.5 bg-brand px-6 md:pr-4 md:pl-5  py-3 text-md font-semibold uppercase leading-normal whitespace-normal text-white transition-colors hover:bg-brand-hover active:bg-brand-hover/90 disabled:cursor-not-allowed disabled:opacity-60 disabled:hover:bg-brand">
